...
def clip_and_normalize(np_image: np.ndarray,
    ...
    '''
    np_image1=np_image
    np_image1=np.transpose(np_image1,(2,1,0))  
    

    
    np_image1=cv2.rotate(np_image, cv2.ROTATE_180)
    np_image2=cv2.rotate(np_image2, cv2.ROTATE_180)
    np_image3=cv2.rotate(np_image3, cv2.ROTATE_180)
    '''
...
def mha2jpg(mhapath):
    ...
    input_image = center_crop(input_image, new_shape=(80,190,190))
...
def data_preprocess(x, labels):
  ...
  img=tf.py_function(mha2jpg,inp=[x],Tout=(tf.float64))
  
  return {'input_1' :img}, {'probCOVID':labels[0], 'probSevere' :labels[1]}
...
def ResNet50(input_shape=(190,190,80)):
  input_1 =Input(shape=(190,190,80),name='input_1')
  ...
  x=tf.keras.layers.DepthwiseConv2D((1,1),padding='same',strides=1,depth_multiplier=3)(input_1)
  ...
  #X4 = convolutional_block(X4, f = 3, filters = [512, 512, 2048], stage = 4, block='g', s = 2)
  '''
  X4 = tf.keras.layers.AveragePooling2D()(X4)
  X4 = tf.keras.layers.Flatten()(X4)
  
  
  X_severe = Dense(1,activation='sigmoid', kernel_initializer=tf.keras.initializers.glorot_uniform(seed=0))(X)
  X_covid = Dense(1,activation='sigmoid', kernel_initializer=tf.keras.initializers.glorot_uniform(seed=0))(X)
  
  X4_severe = Dense(1,activation='sigmoid', kernel_initializer=tf.keras.initializers.glorot_uniform(seed=0))(X4)
  X4_covid = Dense(1,activation='sigmoid', kernel_initializer=tf.keras.initializers.glorot_uniform(seed=0))(X4)
  

  
  
  predictions_severe_= tf.keras.layers.Add()([X_severe*0.75,X4_severe*0.25])
  predictions_covid_= tf.keras.layers.Add()([X_covid*0.75,X4_covid*0.25])
  
  
  age_dense = Dense(1,activation='sigmoid', kernel_initializer=tf.keras.initializers.glorot_uniform(seed=0))(input_age)
  gender_dense = Dense(1,activation='sigmoid', kernel_initializer=tf.keras.initializers.glorot_uniform(seed=0))(input_gender)
  
  
  predictions_severe_ = tf.keras.layers.Add()([X_severe*0.8,X4_severe*0.1,age_dense*0.05,gender_dense*0.05])
  predictions_covid_  = tf.keras.layers.Add()([X_covid*0.8,X4_covid*0.1,age_dense*0.05,gender_dense*0.05])
  ...
  predictions_severe_ = Dense(1, activation='sigmoid',kernel_initializer=tf.keras.initializers.glorot_uniform(seed=0),name='probSevere')(X) #multi-class
  predictions_covid_ = Dense(1, activation='sigmoid', kernel_initializer=tf.keras.initializers.glorot_uniform(seed=0),name='probCOVID')(X)
  
  
  
  
  model = tf.keras.Model(inputs=[input_1], outputs=[predictions_severe_,predictions_covid_])   
  return model
...
def BCEE2(y_true,y_pred):
  ...
  pred_diff=1-y_pred
  label_diff=1-y_true
  result= tf.reduce_sum(-y_true*tf.math.log(y_pred+1e-7)*5-label_diff*tf.math.log(pred_diff+1e-7))/float(len(y_true))
...
def BCEE2_(y_true,y_pred):
  ...
  pred_diff=1-y_pred
  label_diff=1-y_true
  result= tf.reduce_sum(-y_true*tf.math.log(y_pred+1e-7)-label_diff*tf.math.log(pred_diff+1e-7)*1.7)/float(len(y_true))
...
def play(train_dataset, val_dataset):
  ...
  model.compile(optimizer=tf.keras.optimizers.Adam(learning_rate=0.001), loss={'probCOVID': BCEE2_, 'probSevere':BCEE2},metrics=[tf.keras.metrics.AUC(curve='ROC', name='auc_pr')])
  callbacks = [
    ...
    tf.keras.callbacks.ReduceLROnPlateau(monitor='val_loss', factor=0.5, patience=50, verbose=1, mode='auto', min_delta=0.0001, cooldown=0, min_lr=0),
]
...
def test(test_dataset):
  ...
  model.load_weights("/home/jhjoo/grand_challenge/best/en/another/7119/ensemble_10.h5")
